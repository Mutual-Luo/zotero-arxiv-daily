...
source:
  arxiv:
    ...
    retrieval_strategy: rss_first # Roll back to the old API-first behavior by changing this to legacy_api.
...
source:
  arxiv:
    ...
    retrieval_strategy: rss_first # Use rss_first to avoid arXiv API rate limits: RSS metadata -> rerank -> full text only for final executor.max_paper_num papers. To roll back to the old behavior, set this to legacy_api.
  biorxiv:
